...
const PendingMaterialReqPurchase = () => {
  ...
  useEffect(() => {
    const fetchPendingRequests = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_URL_API}/materials?status=2,4`, {
          headers: {
          ...
          <tbody>
            {pendingRequests.map((item, index) => (
              <tr key={item.id}> 
                ...
                <td className='text-muted f_14 fw-semibold text-center'><Link to={`/dashboard/purchase/pending-material-requests/details?ticket-id=${item.ticket_id}`}><i class="fa-solid fa-eye text-danger"></i></Link></td>
